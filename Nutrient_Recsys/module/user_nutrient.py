from rec_nutrient import rec_EER, rec_carbonhydrate, rec_protein, rec_water, rec_vita_a, rec_vita_b6, rec_vita_b12, rec_vita_c, rec_vita_d, rec_ca, rec_mg, rec_na, rec_cl, rec_p, rec_k
import numpy as np
import pymysql
...
class User_Nutrient:
    ...
    def get_nutrient(self):
        ...
        water = rec_water(self.state, self.period, self.sex, self.weight, self.trimester, self.postpartum)/1000 #물
        ...
        vit_d = rec_vita_d(self.state, self.period, self.trimester, self.postpartum)/40 #비타민 D
        ca = rec_ca(self.state, self.period, self.trimester, self.postpartum) #칼슘
        na = rec_na(self.state, self.period, self.trimester, self.postpartum)/1000 #나트륨
        p = rec_p(self.state, self.period, self.trimester, self.postpartum) #인
        k = rec_k(self.state, self.period, self.trimester, self.postpartum)/1000 #칼륨
        cl = rec_cl(self.state, self.period, self.trimester, self.postpartum)/1000 #염소
    ...
    def nutrient_percent(self, user_id): # 유저의 권장 섭취량과 유저의 id를 받아 장바구니에 담긴 상품의 영양소를 array로 넣는다.
        ...
        qry = '''
        SELECT nutrient.* FROM nutrient INNER JOIN orders ON nutrient.product_id = orders.product_id WHERE orders.id = {}; 
        '''.format(user_id)
        cursor.execute(qry)
